...
zmien_liczbe_miejsc2(id_wycieczki NUMBER, liczba_miejsc NUMBER) AS
zajete_miejsca NUMBER;
czy_istnieje INT;
BEGIN
 SELECT COUNT(*) INTO czy_istnieje FROM WYCIECZKI w where w.ID_WYCIECZKI =
zmien_liczbe_miejsc2.id_wycieczki;
 IF czy_istnieje = 0 THEN
 raise_application_error(-20002, 'Wycieczka o podanym ID nie istnieje');
 end if;
 SELECT w.LICZBA_MIEJSC - w.LICZBA_WOLNYCH_MIEJSC INTO zajete_miejsca FROM WYCIECZKI
w
 ...
 LICZBA_WOLNYCH_MIEJSC = zmien_liczbe_miejsc2.liczba_miejsc - zajete_miejsca
